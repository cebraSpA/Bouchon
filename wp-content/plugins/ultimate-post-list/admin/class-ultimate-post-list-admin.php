...
class Ultimate_Post_List_Admin {
	...
	public function print_shortcode ( $post ) {
		printf( '<textarea readonly>[%s id="%d"]</textarea>', UPL_SHORTCODE_NAME, $post->ID );
	}
	...
	public function print_infos () {
		printf( "<h3>%s</h3>\n", esc_html__( 'Do you need more options?', 'ultimate-post-list' ) );
		printf( "<p><a href=\"http://shop.stehle-internet.de/downloads/ultimate-post-list/\" target=\"_blank\">%s</a></p>\n", esc_html__( 'Get the Pro version.', 'ultimate-post-list' ) );
	...
	public function add_posts_list_columns ( $columns ) {
		...
		// add column 'cb', take available column, remove existing column from origin
		$key = 'cb';
		if ( isset( $columns[ $key ] ) ) {
			$new_columns[ $key ] = $columns[ $key ];
			unset( $columns[ $key ] );
		} else {
			$new_columns[ $key ] = $headers[ $key ];
		}

		// add column 'title', take available column, remove existing column from origin
		$key = 'title';
		if ( isset( $columns[ $key ] ) ) {
			$new_columns[ $key ] = $columns[ $key ];
			unset( $columns[ $key ] );
		} else {
			$new_columns[ $key ] = __( $headers[ $key ] );
		}

		// add column 'shortcode'
		$new_columns[ 'shortcode' ] = __( 'Shortcode', 'ultimate-post-list' );

		// add column 'author'
		$new_columns[ 'author' ] = __( $headers[ 'author' ] );

		// add column 'date', take available column, remove existing column from origin
		$key = 'date';
		if ( isset( $columns[ $key ] ) ) {
			$new_columns[ $key ] = $columns[ $key ];
			unset( $columns[ $key ] );
		...
		$new_columns[ 'last-modified' ]	= __( $headers[ 'last-modified' ] );
	...
	public function print_posts_list_content ( $column_name, $post_id ) {
		switch ( $column_name ) {
			case 'shortcode':
				...
				break;
			case 'last-modified':
				echo get_the_modified_date( $this->verbose_date_format, $post_id );
				break;
		}
	}
	 ...
	 * @access   public
	 */
	public function save_list_settings( $post_id, $post ) {
		...
		$css_file_path = dirname( dirname( __FILE__ ) ) . '/public/css/ultimate-post-list-public.css';
