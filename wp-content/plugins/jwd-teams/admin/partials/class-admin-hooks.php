...
class JWD_Teams_Admin_Hooks {
	...
	public function get_member_fileds($count, $member = null) {
		...
		$html .= '<h2 class="jt__teams-head">'.$head.'<span class="jt__remove button button-small" title="'.__('Delete this member',$this->textdomain).'">'.$hooks->get_fa_icon('fa-user-times').' &nbsp;'.__('Remove', $this->textdomain).'</span></h2>';
		$html .= '<ul class="jt__teams-list">';
			...
			$html .= '<li class="jt__teams-details">';
				$html .= '<ul class="jt__teams-details-list">';
					$html .= '<li class="jt__teams-details-item mb_name">' . $this->get_member_filed( 'text', null, $mb_name, $name, __( 'Full Name', $this->textdomain), 'fa-user-o' ) . '</li>';
					$html .= '<li class="jt__teams-details-item mb_position">' . $this->get_member_filed( 'text', null, $mb_position, $position, __( 'Position', $this->textdomain), 'fa-building-o' ) . '</li>';
					$html .= '<li class="jt__teams-details-item mb_phone">' . $this->get_member_filed( 'text', null, $mb_phone, $phone, __( 'Phone', $this->textdomain), 'fa-phone' ) . '</li>';
					$html .= '<li class="jt__teams-details-item mb_email">' . $this->get_member_filed( 'text', null, $mb_email, $email, __( 'Email', $this->textdomain), 'fa-envelope-o' ) . '</li>';
					$html .= '<li class="jt__teams-details-item mb_desc">' . $this->get_member_filed( 'textarea', null, $mb_desc, $desc, __( 'Description', $this->textdomain), 'fa-address-card-o' ) . '</li>';
				$html .= '</ul>';
			$html .= '</li>';
			$html .= '<li class="jt__teams-socials">';
				$html .= '<ul class="jt__teams-socials-list">';
					$html .= '<li class="jt__teams-socials-item">' . $this->get_member_filed( 'text', null, $mb_fb, $fb, __( 'Facebook URL', $this->textdomain), 'fa-facebook' ) . '</li>';
					$html .= '<li class="jt__teams-socials-item">' . $this->get_member_filed( 'text', null, $mb_tw, $tw, __( 'Twitter URL', $this->textdomain), 'fa-twitter' ) . '</li>';
					$html .= '<li class="jt__teams-socials-item">' . $this->get_member_filed( 'text', null, $mb_in, $in, __( 'LinkedIn URL', $this->textdomain), 'fa-linkedin' ) . '</li>';
					$html .= '<li class="jt__teams-socials-item">' . $this->get_member_filed( 'text', null, $mb_gp, $gp, __( 'Google+ URL', $this->textdomain), 'fa-google-plus' ) . '</li>';
					$html .= '<li class="jt__teams-socials-item">' . $this->get_member_filed( 'text', null, $mb_yt, $yt, __( 'YouTube URL', $this->textdomain), 'fa-youtube' ) . '</li>';
					$html .= '<li class="jt__teams-socials-item">' . $this->get_member_filed( 'text', null, $mb_it, $it, __( 'Instagram URL', $this->textdomain), 'fa-instagram' ) . '</li>';
				$html .= '</ul>';
			$html .= '</li>';
		$html .= '</ul>';
	...
	private function get_member_filed( $type = '', $args = array(), $name = '', $value = '',  $title = '', $icon = '') {
		...
		switch ($type){
			...
			case 'img':
				...
				$output .= '<input id="jt__teams-attachment" name="'.$args['mb_img'].'" type="text" value="'.$args['img'].'" size="5" />';
				$output .= '<input id="jt__teams-attachmentid" name="'.$args['mb_img_id'].'" type="text" value="'.$args['img_id'].'" size="5"/>';
				$output .= '<span data-jtcontainer="jt__teams-img-'.$args['count'].'" class="jt__changeimg button button-primary" title="'.__('Add Image',$this->textdomain).'">'.$hooks->get_fa_icon('fa-file-image-o').'</span>';
				$output .= '<span data-avatar="'.$args['default_img'].'" class="jt__removeimg button" title="'.__('Delete Image',$this->textdomain).'">'.$hooks->get_fa_icon('fa-trash').'</span>';
			break;
			default: 
