...
class JWD_Teams_Admin {
	...
	public function enqueue_admin() {
		...
		if( $post_type == 'jwd_team' ){
			...
			wp_enqueue_style( $this->textdomain . '-fontawesome', plugin_dir_url( __FILE__ ) . 'css/jwdteams-fontawesome.min.css', array(), $this->version, 'all' );
		...
		wp_localize_script( 'jquery', 'jt__getTeams', 
			array( 
	...
	public function register_metaboxes() {
		$hooks = $this->hooks;
		add_meta_box( 'jwdteams_members', $hooks->get_fa_icon('fa-users') . '&nbsp;' . __( 'Team Members', $this->textdomain ), array( $this, 'team_members' ), 'jwd_team', 'normal', 'high' );
		add_meta_box( 'jwdteams_settings', $hooks->get_fa_icon('fa-cogs') . '&nbsp;' . __( 'Team Settings', $this->textdomain ), array( $this, 'team_settings' ), 'jwd_team', 'side', 'default' );
		add_meta_box( 'jwdteams_custom_css', $hooks->get_fa_icon('fa-css3') . '&nbsp;' . __( 'Team Custom CSS', $this->textdomain ), array( $this, 'team_custom_css' ), 'jwd_team', 'normal', 'default' );
	}
	/**
...
<?php
	...
	public function team_settings( $post ) {
		...
		/* Output COLORS Tab */
		$output .= '<li>';
		$output .= $adminHooks->get_settings_filed('mb_name_color', $mb_name_color_args, null, null, 'fa-user-o', __( 'Name', $this->textdomain ) );
		$output .= $adminHooks->get_settings_filed('mb_position_color', $mb_position_color_args, null, null, 'fa-building-o', __( 'Position', $this->textdomain ) );
		$output .= $adminHooks->get_settings_filed('mb_contact_color', $mb_contact_color_args, null, null, 'fa-envelope-o', __( 'Phone & Email', $this->textdomain ) );
		$output .= $adminHooks->get_settings_filed('mb_social_color', $mb_social_color_args, null, null, 'fa-share-square-o', __( 'Social Icons', $this->textdomain ));
		$output .= $adminHooks->get_settings_filed('mb_social_hover', $mb_social_hover_args, null, null, 'fa-share-square', __( 'Social Icons Hover', $this->textdomain ) );
		$output .= $adminHooks->get_settings_filed('mb_desc_color', $mb_desc_color_args, null, null, 'fa-address-card-o', __( 'Description', $this->textdomain ));
		$output .= $adminHooks->get_settings_filed('mb_tooltip_color', $mb_tooltip_color_args, null, null, 'fa-commenting-o', __( 'Tooltip Text', $this->textdomain ));
		$output .= $adminHooks->get_settings_filed('mb_tooltip_bg', $mb_tooltip_bg_args, null, null, 'fa-commenting', __( 'Tooltip Background', $this->textdomain ));
		$output .= '</li>';
		/* Output SIZES Tab */
		$output .= '<li>';
		$output .= $adminHooks->get_settings_filed('mb_name_size', $mb_name_size_args, null, null, 'fa-user-o',  __( 'Name', $this->textdomain ) . ' <i style="font-size:90%;font-weight:normal;">(px)</i>');
		$output .= $adminHooks->get_settings_filed('mb_position_size', $mb_position_size_args, null, null, 'fa-building-o', __( 'Position', $this->textdomain ) . ' <i style="font-size:90%;font-weight:normal;">(px)</i>');
		$output .= $adminHooks->get_settings_filed('mb_contact_size', $mb_contact_size_args, null, null, 'fa-envelope-o', __( 'Phone & Email', $this->textdomain ) . ' <i style="font-size:90%;font-weight:normal;">(px)</i>');
		$output .= $adminHooks->get_settings_filed('mb_social_size', $mb_social_size_args, null, null, 'fa-share-square-o', __( 'Social Icons', $this->textdomain ) . ' <i style="font-size:90%;font-weight:normal;">(px)</i>');
		$output .= $adminHooks->get_settings_filed('mb_desc_size', $mb_desc_size_args, null, null, 'fa-address-card-o', __( 'Description', $this->textdomain ) . ' <i style="font-size:90%;font-weight:normal;">(px)</i>');
		$output .= $adminHooks->get_settings_filed('mb_desc_leng', $mb_desc_leng_args, null, null, 'fa-address-card-o', __( 'Description Length', $this->textdomain ) . ' <i style="font-size:90%;font-weight:normal;">('.__('characters',$this->textdomain).')</i>' );
		$output .= '</li></ul>';
	...
	public function custom_admin_version($text) {
		...
		if ('jwd_team' == $post_type){ 
			return $this->fullname . ' v' . $this->version . '&nbsp;' . sprintf ( __('%1$s %2$s',$this->textdomain), __('Powered by', $this->textdomain) ,'<a href="http://jordachewd.com/" target="blank">JordacheWD</a>'); 
		}
		return $text;
	}
	/**
