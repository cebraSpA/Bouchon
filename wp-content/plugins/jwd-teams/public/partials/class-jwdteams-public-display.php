...
class JWD_Teams_Public_Display {
	...
	public function get_tipso_script($args){
		ob_start();
		$tipso_id = $tipso_bg = $tipso_txt = $output = '';
		if( isset($args['tipso_id']) && !empty($args['tipso_id'])){ $tipso_id = $args['tipso_id']; } 
		if( isset($args['tipso_bg']) && !empty($args['tipso_bg'])){ $tipso_bg = $args['tipso_bg']; } 
		if( isset($args['tipso_txt']) && !empty($args['tipso_txt'])){ $tipso_txt = $args['tipso_txt']; } 
		if( $tipso_id != '' ){ 
	...
	public function get_team_style($id){
		...
		if(empty($id)){ return '<p style="color:red">ERROR: Parameter <b>$id</b> is missing in <b>get_team_style()</b>! <u> '.plugin_basename(__FILE__).' </u></p>'; }
		/* Vars */
