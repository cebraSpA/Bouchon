...
class JWD_Teams_Public {
	 ...
	 * @since    1.3.5
	 */
	public function public_shortcode() {
		add_shortcode( 'jwd_team', array( $this, 'shortcode') );
	}
		...
		if( $atts['id'] != '' ){
			...
			if ( FALSE === get_post_status( $atts['id'] ) ) {
			  $output .= '<div class="jt__error">'. sprintf ( __( 'Sorry! No team registered with this ID: %s', $this->textdomain ) ,'<b>'.$atts['id'].'</b>' ).'</div>';
				...
				/* VARS */
				$member_data 				= get_post_meta( $atts['id'], 'member_data', true );
				$team_custom_css			= get_post_meta( $atts['id'], 'team_custom_css', true); 
						...
						if ( isset($member['img']) || isset($member['img_id']) || isset($member['name']) || isset($member['position']) || isset($member['phone']) || isset($member['email']) || isset($member['desc']) ){
							...
							$output .= '<ul class="jt__member '.esc_attr( $team_txt_align ).' ">';
								if( $team_settings_desc == 'member_desc_tooltip' && $member['desc'] != ''){
									$output .= '<li class="jt__member-img jt__team_tooltip_'.esc_attr( $atts['id'] ).'" data-tipso-title="'.esc_attr( $member['name'] ).'" data-tipso="'.$member['desc'].'">';
								} else { $output .= '<li class="jt__member-img">'; }
								if( !empty($member['img_id'])){
									$output .= wp_get_attachment_image($member['img_id'], 'large', false, array( "class" => esc_attr( $team_settings_img) ));
								} elseif( !empty($member['img']) ) {
									$output .= '<img src='.esc_url( $member['img'] ).' alt="'.esc_attr( $member['name'] ).'" class="'.esc_attr( $team_settings_img).'" />';
								} else {
									$output .= '<img src='. plugins_url('/public/img/jwd_team_avatar.gif', dirname(__FILE__)) .' alt="'.esc_attr( $member['name'] ).'" class="'.esc_attr( $team_settings_img).'" />';
								}
								$output .= '</li>'; 
								...
								if($member['phone'] != ''){ $output .= '<li class="jt__member-phone">'.$hooks->get_fa_icon('fa-phone', $icon_style) . esc_attr( $member['phone'] ).'</li>'; }
								if($member['email'] != ''){ $output .= '<li class="jt__member-email">'.$hooks->get_fa_icon('fa-envelope', $icon_style) . esc_attr( $member['email'] ).'</li>'; }
								if( $member['fb'] != '' || $member['tw'] != '' || $member['in'] != '' || $member['gp'] != '' || $member['yt'] != '' || $member['it'] != ''){
									 $output .= '<li class="jt__member-socials">';
										if($member['fb'] != ''){ $output .= '<a href="'.esc_url($member['fb']).'" target="blank">'.$hooks->get_fa_icon('fa-facebook').'</a>'; }
										if($member['tw'] != ''){ $output .= '<a href="'.esc_url($member['tw']).'" target="blank">'.$hooks->get_fa_icon('fa-twitter').'</a>'; }
										if($member['in'] != ''){ $output .= '<a href="'.esc_url($member['in']).'" target="blank">'.$hooks->get_fa_icon('fa-linkedin').'</a>'; }
										if($member['gp'] != ''){ $output .= '<a href="'.esc_url($member['gp']).'" target="blank">'.$hooks->get_fa_icon('fa-google-plus').'</a>'; }
										if($member['yt'] != ''){ $output .= '<a href="'.esc_url($member['yt']).'" target="blank">'.$hooks->get_fa_icon('fa-youtube').'</a>'; }
										if($member['it'] != ''){ $output .= '<a href="'.esc_url($member['it']).'" target="blank">'.$hooks->get_fa_icon('fa-instagram').'</a>'; }
									 $output .= '</li>';
								}
				...
				if( $team_settings_credit != 'hide'){ 
					$output .= '<p class="jt__credit">' . __('Powered by', $this->textdomain) . ' <a href="http://jordachewd.com/" target="blank">JordacheWD</a></p>';
				}
				/* Get Tipso Script */
				$output .= $this->display->get_tipso_script( array( 'tipso_id' => esc_attr( $atts['id'] ), 'tipso_bg' => $mb_tooltip_bg, 'tipso_txt' => $mb_tooltip_color ));	
				$output .= '</div>';
			}
		} else {
