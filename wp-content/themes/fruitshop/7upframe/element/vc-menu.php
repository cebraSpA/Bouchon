...
if(!function_exists('s7upf_vc_menu'))
{
    function s7upf_vc_menu($attr,$content = false)
    {
        ...
        $html = $style = $active_color = $extra_class=$menu_fixed = $data_add_social = $img_logo = $bg_color = $active_color = $text_color = $add_social = $menu = $cart_box = $search_box = $class_menu = $menu_position = '' ;
                ...
                if(!empty($img_logo)){
                    $html .= '<div class="col-md-3 col-sm-12 col-xs-12">
							<div class="logo logo3 pull-left">
								<a href="'.esc_url(get_home_url('/')).'">'.wp_get_attachment_image($img_logo,'full').'</a>
							</div>
						</div>';
                    $col = '9';
                }
