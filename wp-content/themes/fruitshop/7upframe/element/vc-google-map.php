        ...
        if(is_array($location) && count($location) > 0 ) {
            foreach ($location as $value) {
                if(!empty($value)) {
                    ...
                    $location_text .= '|' . s7upf_get_data_isset($value,'st_lat') . '&&' . s7upf_get_data_isset($value,'st_long') . '&&' . s7upf_get_data_isset($value,'marker_title') . '&&' . s7upf_get_data_isset($value,'info_box') . '&&' . (($img_src !='' )?$img_src:'');
                }
            }
        }
        $img = array();$img[0]='';
