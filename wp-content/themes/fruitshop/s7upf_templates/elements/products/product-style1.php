...
<div class="<?php echo esc_attr($class_bg); ?> mb_bg_product_style1">
    <div class="container">
        <?php } ?>
        <?php if(!empty($title)) {?><h2 class="title30 font-bold title-box1 text-center"><?php echo esc_attr($title); ?></h2><?php } ?>
    <?php
        if($pro_query->have_posts()){ ?>

        <div class="<?php echo esc_attr($extra_class); ?> featured-product-slider mb-element-product-style1 woocommerce <?php echo esc_attr($class_main_color);?> <?php echo esc_attr($class_secondary_color);?>">
            <div class="wrap-item group-navi" data-navigation="true" data-pagination="false" data-autoplay="<?php echo esc_attr($autoplay); ?>" data-itemscustom="[[0,1],[990,<?php echo esc_attr($number_item_desktop)?>]]">
