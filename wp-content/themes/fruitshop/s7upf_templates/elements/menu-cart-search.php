    ...
    <?php if('show' === $search_box){ ?>
            <li class="live-search-on">
                <form class="search-form search-form3 pull-right" action="<?php echo esc_url(home_url('/'))?>">
                    <input name="s" value="<?php echo get_search_query()?>" type="text" placeholder = "<?php echo esc_html__('Search this site','fruitshop');?>">
                    <input name="post_type" type="hidden" value="product">
                    <input type="submit" value="">
                </form>

            </li>
        <?php } if('show' === $cart_box and class_exists('WC_Product')){
            ?>
            <li>
