...
if (!empty($media_url)) {
    if(strpos($media_url,'.mp3') !== false){
        $data .='<div class="media-audio-blog single-thumbnail banner-adv "><audio controls><source src="'.esc_url($media_url).'" type="audio/mpeg"></audio>';
    }else{
        $data .= '<div class="audio media-audio-blog single-thumbnail banner-adv ">' . s7upf_remove_w3c(wp_oembed_get($media_url, array('height' => '176'))) . '</div></div>';
    }
}
?>
