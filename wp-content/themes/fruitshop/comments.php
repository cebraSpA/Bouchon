...
<div id="comment" class="mb-comment single-comment">
        <?php if ( have_comments() ) : ?>
            ...
            <?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through ?>
                <nav id="comment-nav-below" class="comment-navigation" >
                    <h2 class="title30 font-bold"><?php esc_html_e( 'Comment navigation', 'fruitshop' ); ?></h2>
                    <div class="nav-previous"><?php previous_comments_link( esc_html__( '&larr; Comentarios anteriores', 'fruitshop' ) ); ?></div>
                    <div class="nav-next"><?php next_comments_link( esc_html__( 'Comentarios recientes &rarr;', 'fruitshop' ) ); ?></div>
        ...
        if ( ! comments_open() && '0' != get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
            ?>
            <p class="no-comments"><?php esc_html_e( 'Los comentarios estan cerrados.', 'fruitshop' ); ?></p>
        <?php endif; ?>

        <?php
        $commenter = wp_get_current_commenter();
        $comment_form = array(
            'title_reply'          => have_comments() ? esc_html__( 'Deja un comentario', 'fruitshop' ) : esc_html__( 'Se el primero en comentar', 'fruitshop' ) . ' &ldquo;' . get_the_title() . '&rdquo;',
            'title_reply_to'       => esc_html__( 'Dejar una respuesta a %s', 'fruitshop' ),
            ...
            'label_submit'  => esc_html__( 'Comentar', 'fruitshop' ),
            'logged_in_as'  => '',
            'comment_field' => '<div class="col-md-6 col-sm-6 col-xs-12"><label>'.esc_html__('Comentario *','fruitshop').'</label><textarea id="comment-textarea" name="comment" class="mb-textarea border" cols="40" rows="5"></textarea></div>',
